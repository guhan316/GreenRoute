...
class SupabasePersistence:
    ...
    async def save_optimization(self, token: str, payload: dict[str, Any]) -> str:
        ...
        vehicle_details = optimization.get('vehicle') or form.get('vehicle') or {}
        rpc_payload = {
            'p_origin_label': origin.get('address') or origin.get('label') or str(form.get('origin')),
            'p_destination_label': destination.get('address') or destination.get('label') or str(form.get('destination')),
            'p_origin_lon': origin['lon'],
            'p_origin_lat': origin['lat'],
            'p_destination_lon': destination['lon'],
            'p_destination_lat': destination['lat'],
            'p_vehicle_key': vehicle_details.get('key') or form.get('vehicle_type') or 'custom',
            'p_load_kg': form['load_kg'],
            'p_fuel_price_per_litre': form.get('fuel_price_per_litre', 0),
            ...
            'p_vehicle_details': vehicle_details,
        }
    ...
    async def get_history(self, token: str, limit: int = 20) -> list[dict[str, Any]]:
        await self.get_user(token)
        params = {
            'select': 'id,origin_label,destination_label,vehicle_key,vehicle_manufacturer,vehicle_model,manufacture_year,emission_stage,fuel_type,base_mileage_kmpl,max_payload_kg,kerb_weight_kg,load_kg,fuel_price_per_litre,departure_time,routing_mode,selected_strategy,created_at,route_candidates(id,provider_candidate_id,distance_km,duration_minutes,traffic_delay_minutes,fuel_litres,fuel_cost,co2_kg,balanced_score,recommended_as)',
